...
class login
{
    ...
    public function init($request,$json=''){
        switch ($request) {
            case 'login':
                $id = $this->login($json['username'],$json['password'],"member_register");
                if($id){
                    $sql = "SELECT * FROM member_register WHERE id = '$id'";
                    return json_decode($this->get($sql),true);
                }else{
                    return false;
                }
    ...
    ///////////// Function Main
    public function login($username,$password,$table="member_register"){
        $db = $this->db;
        $db->Disconnect();
        if($db->Connect()){
            $idlogin = $db->login($username,$password,$table);
           if($idlogin!=false){
                return intval($idlogin);
           }else{
               return false;
           }
        }else{
