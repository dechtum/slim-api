...
class shop
{
    ...
    public function init($request,$json=''){
        switch ($request) {
            case 'create':
                $newIdRow = $this->creaeteShop($json,$json['action']);
                return $newIdRow;
                break;
            case 'update':
    ...
    ///////////// Function Main
    public function creaeteShop($json,$table=''){  // FOR  INSERT UPDATE
        $db = $this->db;
        $dbMage =$this->dbMage;
        $db->Disconnect();
                ...
                if($db->CheckTable($sql)){
                    ...
                    if($db->Iquery($sql)){
                      //  echo var_dump($db->GetlastID());
                        return $this->createTableSHOP("SHOP",$json['registerId'],$db->GetlastID());
                    }else{
                        return false;
                    }
                }else{
    ...
    public function getlistshop($json=''){
        $sql ="";
        if($json['shopId'][0]=="" || $json['shopId'] == ""){
            $sql = "SELECT * FROM member_shop";
        }else if($json['shopId'][0]!=""){
            $wh = "";
            foreach ((array)$json['shopId'] as $key => $value) {
                if(count((array)$json['shopId'])-1 == $key){
                    $wh .= " id = '$value' ";
                }else{
                    $wh .= " id = '$value' OR";
                }
            }
            $sql = "SELECT * FROM member_shop WHERE ".$wh;
        }

        $db = $this->db;
        $db->Disconnect();
        $idreg = $json['registerId'];
        $db->database = "{$idreg}_MEMBER";
        if($db->Connect()){
           if($db->CheckRow($sql)>0){
    ...
    public function createTableSHOP($database,$uid,$shopid){
        $db = $this->db;
        $dbMage =$this->dbMage;
        $db->Disconnect();
        if($db->ConnectNoDatabase()){
            $databseName = "{$uid}_{$shopid}_{$database}";
            if($db->showDatabase($databseName)==false){
                if($db->CreateDB($databseName)){
                    ...
                    $db->Disconnect();
                    if($db->Connect()){
                        $member = file_get_contents(__DIR__."/../../mysql_admin/shop.sql");
